% Replicate Figure 3 with a standalone MATLAB entry point.
close all; clc;
...
fprintf("Figure 3 MATLAB replication\n");
[Q, S] = flo_sample_attributes(N, SCENARIO, SEED_ATTR);
pos = Q > 0;
neg = Q < 0;

cases = {
    "Memory", C_MEM, D_MEM, 301;
    "No-memory", C_NOM, D_NOM, 302
};
...
z = linspace(-1, 1, 600);
mediaPdf = flo_shifted_beta_pdf(z, 1, 8);

fig = figure("Units", "inches", "Position", [1, 1, 10.8, 4.2], "Color", "w");
tl = tiledlayout(fig, 1, 2, "TileSpacing", "compact", "Padding", "compact");
title(tl, "Figure 3 replication: skewed media independent of Q", "FontName", "Helvetica", "FontSize", 12, "Color", colors.dark);
...
outBase = fullfile(repoRoot, "figures_matlab", "fig3_replication_matlab");
