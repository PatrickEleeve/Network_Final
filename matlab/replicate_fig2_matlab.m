% Replicate Figure 2 with a standalone MATLAB entry point.
close all; clc;
...
fprintf("Figure 2 MATLAB replication\n");
[Q, S] = flo_sample_attributes(N, SCENARIO, SEED_ATTR);
pos = Q > 0;
neg = Q < 0;

cases = {
    "Memory", C_MEM, D_MEM, 201;
    "No-memory", C_NOM, D_NOM, 202
};
...
title(tl, "Figure 2 replication: polarized media independent of Q", "FontName", "Helvetica", "FontSize", 12, "Color", colors.dark);
...
for j = 1:2
    ...
    xline(ax, -1, "--", "Color", colors.gray, "LineWidth", 1.4, "DisplayName", "media support");
    xline(ax, 1, "--", "Color", colors.gray, "LineWidth", 1.4, "HandleVisibility", "off");
    xlim(ax, [-1, 1]);
    ylim(ax, [0, 55]);
...
outBase = fullfile(repoRoot, "figures_matlab", "fig2_replication_matlab");
