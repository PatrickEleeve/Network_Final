% Replicate Figure 6 with a standalone MATLAB entry point.
close all; clc;
...
fprintf("Figure 6 MATLAB replication\n");
[Q, S] = flo_sample_attributes(N, SCENARIO, SEED_ATTR);
pos = Q > 0;
neg = Q < 0;

cases = {
    "Memory", C_MEM, D_MEM, 601;
    "No-memory", C_NOM, D_NOM, 602
};
...
fprintf("  Proposition 6 check: Var(memory)=%.6f, Var(no-memory)=%.6f\n", ...
    results{1}.stats.var, results{2}.stats.var);
...
title(tl, "Figure 6 replication: memory reduces stationary variance", ...
...
for j = 1:2
    ...
    hold(ax, "on");
    R = results{j}.R;
    flo_hist_pdf(ax, R(neg), bins, colors.blue, 0.55, "Q = -1");
    flo_hist_pdf(ax, R(pos), bins, colors.red, 0.55, "Q = +1");
    xlim(ax, [-1, 1]);
...
outBase = fullfile(repoRoot, "figures_matlab", "fig6_replication_matlab");
