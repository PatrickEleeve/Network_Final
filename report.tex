...
Figure~4,
$\mathrm{Var}(R^*)=0.1460$ in the memory case, compared with the paper's
reported value near $0.1484$, and the conditional means are
$0.3738$ and $-0.3646$ for the two opinion groups.  The variation studies
...
Source Fig. 2 & $(0.500,0.001)$ & $(0.998004,0.001996)$ & $\{-1,+1\}$ & 8000 \\
Source Fig. 3 & $(0.500,0.001)$ & $(0.998004,0.001996)$ & $-1+2\mathrm{Beta}(1,8)$ & 8000 \\
...
memory case, we obtain $\mathrm{Var}(R^*)=0.1460$; the target reported in the
paper is approximately $0.1484$.  The conditional means are
...
conditional means $0.3679$ and $-0.3627$.  These results support the paper's
main polarization mechanism: when $d$ is large enough and
...
benchmark, source-paper Figure~4, is close to the reported values.  This gives
confidence that our implementation captures the intended recursion rather than
only producing visually similar plots.
...
Our own study also has limits.  Most experiments are single-seed reproductions
or parameter scans over one graph size, so they should not be read as
asymptotic empirical laws.  The power-law configuration model removes
...
Main implementation assumptions are equal inbound-neighbor weights, fixed random
seeds, independently sampled media signals at each time step, conservative
iteration counts, and independent implementation from the equations in the
paper rather than from author code.
